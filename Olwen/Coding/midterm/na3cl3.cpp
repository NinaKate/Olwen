#include "nacltake2.hpp"


int main()
{
    string name("Na3Cl3");      // for output files
    const int nNa = 3, nCl = 3;
    const int n = nNa + nCl;
    double a = 0.2;
    double rt2 = 1.4;
    //first config, just a line
    double r_Na[nNa][3] = { { 0, 0, 0 }, { 2*a, 0, 0},{4*a,0,0} };
    double r_Cl[nCl][3] = { { a, 0, 0 }, {  3*a, 0, 0}, {5*a,0,0}};

    // Initialize the cluster, add guesses at the
    // minimum arrangement. 
    Cluster cluster(10000,0.001);
    for (int i = 0; i < nNa; i++) {
    ...
    file << "set title \"(NaCl)_3 Energy = " << pe / 2 << " eV\"\n"
         << "set hidden3d\n"
         << "unset key\n"
         << "splot \"na3cl3.data\" with points lt 3 pt 7 ps 8\n"
