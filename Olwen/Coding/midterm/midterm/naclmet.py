...
class Cluster :
    ...
    def MetropolisStep(self,delta):
        p = self.get_variables()
        
    #choose a random variable
        cp = self.potential_energy( )
        n = random.randrange(self.number_of_variables())
        ptest = p #just initializing that
        ptest[n] = p[n] + delta*random.uniform(-1, 1)
        self.set_variables(ptest)
        cptest = self.potential_energy()
        ratio = cptest/cp
        #apply metropolis test
        if ratio**2 > 1.0 or ratio**2 > random.random():
            p[n]=ptest[n]
                #return True
                #else:
                #return False
    ...
    def MetropolisMinimize(self,Steps,walkers):
        delta = 0.0024
        #delta = 0.00024 #gets line better
