...
public:
    ...
    Cluster(double isteps,double idelta): steps(isteps),delta(idelta)
    {
    }
    ...
    void MetropolisStep(){
        ...
        Matrix<double,1> ptest = p;
        //double delta = 0.001;
        int N = number_of_variables();
        //pick random ion coordinate to step
        int n = int(rng.rand()*N);
        //offset the thing by delta
        ptest[n] += delta*(rng.rand_gauss()-0.5);
        
        //Metropolis test
        if (rho(ptest,p)>rng.rand()){
            p = ptest;
        }
        set_variables(p);
    }
    void OneMonteCarloStep(){
        int N = steps; //number of metropolis steps
        for (int i=0;i<N;i++){
            MetropolisStep();
        }
    }
    double minimize(){
        ...
        Matrix<double,1> win = get_variables();
        int count = 0;
        while (count<10){
            OneMonteCarloStep();
            pe =potential_energy();
